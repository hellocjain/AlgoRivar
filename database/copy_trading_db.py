...
class TradeQuality(Base):
    """Trade quality margin percentage settings for dynamic risk sizing (Grade A/B/C/D)."""

    __tablename__ = "copy_trade_qualities"

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, nullable=True, default=1, index=True)
    quality_grade = Column(String(10), nullable=False, unique=True, index=True)  # A | B | C | D
    margin_percentage = Column(Float, default=50.0, nullable=False)              # e.g. 70.0 for 70%
    margin_source = Column(String(20), default="available", nullable=False)      # available | cash
...
class StrategyLeg(Base):
    """Legs defining multi-leg strategies (Straddles, Strangles, Spreads)."""

    __tablename__ = "copy_strategy_legs"

    id = Column(Integer, primary_key=True, autoincrement=True)
    strategy_id = Column(Integer, ForeignKey("copy_strategies.id", ondelete="CASCADE"), nullable=False, index=True)
    leg_number = Column(Integer, nullable=False)
    instrument_type = Column(String(10), default="OPTIDX", nullable=False)  # FUT | CE | PE | OPTIDX
    strike_selection = Column(String(20), default="ATM", nullable=False)     # ATM | ITM1 | OTM1 | FIXED
    action = Column(String(10), default="BUY", nullable=False)               # BUY | SELL
    target_lots = Column(Integer, default=1, nullable=False)
    sl_points = Column(Float, default=0.0)
    target_points = Column(Float, default=0.0)
    created_at = Column(DateTime, default=datetime.utcnow)

    strategy = relationship("CopyStrategy", back_populates="legs")
...
# Update CopyStrategy to link StrategyLeg and add risk fields
CopyStrategy.risk_profile = Column(String(30), default="grade_B")
CopyStrategy.market_condition = Column(String(30), default="any")
CopyStrategy.legs = relationship("StrategyLeg", back_populates="strategy", cascade="all, delete-orphan")
CopyAccount.user_id = Column(Integer, nullable=True, default=1, index=True)
...
def init_copy_trading_db():
    ...
    try:
        ...
        get_or_create_default_trade_qualities()
...
def get_or_create_default_trade_qualities(user_id: int = 1) -> List[Dict[str, Any]]:
    """Ensure default Grade A/B/C/D trade qualities exist."""
    session = Session()
    try:
        defaults = [
            ("A", 70.0, "available"),
        ...
        results = []
        for grade, pct, src in defaults:
            tq = session.query(TradeQuality).filter_by(quality_grade=grade).first()
            if not tq:
                tq = TradeQuality(user_id=user_id, quality_grade=grade, margin_percentage=pct, margin_source=src)
                session.add(tq)
                session.commit()
            results.append(tq.to_dict())
        return results
    except Exception as e:
        session.rollback()
        logger.error(f"Error getting/creating default trade qualities: {e}")
        return []
    finally:
        session.close()


def get_all_trade_qualities(user_id: int = 1) -> List[Dict[str, Any]]:
    """Retrieve all trade quality settings."""
    session = Session()
    try:
        tqs = session.query(TradeQuality).all()
        if not tqs:
            return get_or_create_default_trade_qualities(user_id)
        return [tq.to_dict() for tq in tqs]
    finally:
        session.close()
