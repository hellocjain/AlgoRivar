...
def resolve_active_contract_symbol(symbol: str, exchange: str) -> str:
    ...
    try:
        ...
        if cached_matches:
            # 1st Priority: Active FUTCOM / Futures contracts
            fut_matches = [
                m for m in cached_matches
                if str(m.get("instrumenttype", "")).upper() in ["FUTCOM", "FUTIDX", "FUTSTK", "FUT"]
...
def calculate_child_quantity(
    ...
    if mode == "DYNAMIC_MARGIN" or account.get("risk_profile"):
        try:
            from services.margin_calculator import MarginCalculator
            mc = MarginCalculator()
            risk_prof = account.get("risk_profile", "grade_B")
            base_lots = max(1, master_qty // lot_size) if lot_size > 1 else master_qty
            dyn_lots, _ = mc.calculate_lots_for_account(
                account=account,
                symbol=symbol,
                action="BUY",
            ...
            target_qty = dyn_lots * lot_size
        except Exception as mc_err:
            logger.warning(f"[Copy Trading] Dynamic margin sizing fallback: {mc_err}")
            raw_qty = master_qty * multiplier
            target_qty = max(lot_size, int(round(raw_qty / lot_size) * lot_size))
    elif mode == "FIXED_LOTS" and fixed_qty > 0:
